...
def load_train_data(X, train_size=0.8):
...
def train(X,n_estimators=20,model='Random Forest',kernel='linear'):
    # define regressor
    #model='Random Forest'
    #model='Linear Regression' 
    X_train, X_valid, y_train, y_valid = load_train_data(X)
...
def example():
    ...
    # first column = y
    # second to end = x
    Xtrain = np.random.rand(40,4)

    # start training
    #regr,score = train(Xtrain,model='SVM')
    regr,score = train(Xtrain,model='Random Forest',n_estimators=1)
    print("CV Score = "+str(score))
